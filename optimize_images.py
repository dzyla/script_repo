...
parser.add_argument("--d", type=int, default=1920, help="Maximum dimension of a photo")
parser.add_argument(
    "--s", type=float, default=0.4, help="Maximum size in MB of a photo"
)
parser.add_argument(
    "--e", type=str, help="file extension, only one accepted", required=True
)
parser.add_argument("--q", type=int, default=75, help="JPG save quality")
parser.add_argument("--j", type=int, default=8, help="Number of parallel threads")
parser.add_argument(
    "--m", action="store_true", help="Silent (mute) mode. Show only final stats."
)
parser.add_argument(
    "--b",
    type=str,
    default=False,
    help="Backup dir. Provide the path of the backup dir. "
...
files = []
for file_extension_mod in [
    file_extension,
    file_extension.capitalize(),
    file_extension.lower(),
]:
    files = files + glob.glob(
        input_folder + "/**/*.{}".format(file_extension_mod), recursive=True
    )
...
def write_exif(path, exif_holder):
    metadata_new = pyexiv2.ImageMetadata(path)
    metadata_new.read()
...
def run_job(file):
    copied = False
    folder_new = (
                     os.path.dirname(file).replace(os.path.dirname(input_folder), out_folder)
                 ) + "/"
    ...
    ### Process file only if not present in the output directory or there is a backup folder option
    ...
    if not os.path.exists(path_new) or args.b:
        ...
        try:
            ...
            ### If image was successfully opened, continue
            if im:
                ...
                if im_shape[0] > max_shape[0] or im_shape[1] > max_shape[1]:
                    ...
                    ### Get metadata from the file only if going to be modified before saving a new file
                    exif_holder = get_exif(file)
                ...
                ### if image size is already smaller
                else:
                    try:
                ...
                ### Write new exif
                if exif_holder:
                    write_exif(path_new, exif_holder)

                if not args.m:
                    size_diff = abs(round(img_size - img_size_new, 2))
                    if not copied:
                        print(
                            "Saved {}. Saved space = {} MB.".format(path_new, size_diff)
                        )
                    else:
                ...
                print("Image: {} could not be opened".format(file))
        ...
        return round(float(os.path.getsize(file)) / 1e6, 2) - round(
            float(os.path.getsize(path_new)) / 1e6, 2
        )


t1 = time.time()
results = Parallel(n_jobs=args.j)(
    delayed(run_job)(queue_element) for queue_element in files
)

print(
    "Final statistics: saved space: {} MB, took: {} s.".format(
        round(sum(list(filter(None, results))), 2), round(time.time() - t1), 2
    )
)
